...
async function seed() {
  ...
  await addLedgerEntry({ guildId: g.id, kind: "deposit", amount: "2.00", txId: "dep_1", status: "confirmed" });
  return g;
}
...
describe("reconcilePendingPayouts", () => {
  it("flips a pending payout to completed when Dakota reports completed (and deducts balance)", async () => {
    const g = await seed();
    await addLedgerEntry({ guildId: g.id, kind: "payout", amount: "1.00", txId: "tx_1", status: "pending" });
    expect((await getBalance(g.id)).usdc).toBe("2.00"); // pending payout doesn't yet count

    await reconcilePendingPayouts(fakeClient("completed") as never, g.id);

    expect((await listLedger(g.id)).find((e) => e.txId === "tx_1")!.status).toBe("completed");
  ...
  it("marks a returned/failed payout as failed (balance untouched)", async () => {
    ...
    expect((await listLedger(g.id)).find((e) => e.txId === "tx_1")!.status).toBe("failed");
  ...
  it("leaves a still-in-flight payout pending", async () => {
    ...
    expect((await listLedger(g.id)).find((e) => e.txId === "tx_1")!.status).toBe("pending");
  });

  it("only queries pending payouts — ignores deposits and settled payouts", async () => {
    const g = await seed();
    await addLedgerEntry({ guildId: g.id, kind: "payout", amount: "1.00", txId: "tx_done", status: "completed" });
